...
export default function NewTicket() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        const fOrder: string[] = Array.isArray((s as any)?.formOrder) ? ((s as any).formOrder as string[]).map(String) : []
        setFormOrder(fOrder)
      } catch (err) { void err }
    })()
  }, [])
  ...
  useEffect(() => {
    ...
    const builtins: FormField[] = [
      { id: 'builtin:name', name: 'name', label: byName['name']?.label || 'Seu nome', type: 'select', optionsFromUsers: true, required: Boolean(byName['name']?.required ?? true), order: 0, isActive: (byName['name']?.isActive ?? true) as boolean, visiblePublic: true, visibleInternal: true, readonlyInternal: false },
    ...
    const merged: FormField[] = builtins.map(b => {
      ...
      // Se encontrou config customizada, preserva type/optionsFromUsers do builtin se o customizado for 'text' mas quisermos forçar select para name
      // Mas a logica original era { ...b, ...found }. Se found tem type='text', sobrescreve.
      // O usuario quer que seja select. Entao vamos forçar se for o campo name.
      if (b.name === 'name') {
